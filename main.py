...
def reset_env():
    ...
    update_last_action([0 for _ in range(ACTION_SPACE_DIM)])
...
def get_reward(past_position, new_position, epoch):
    ...
    if foot_contact_readings[0] and foot_contact_readings[1]:
    ...
    if can_plot_rewards(epoch):
        rewards_data_plot.append({
            "robot_height_reward": robot_height_reward,
            "speed": speed,
...
for epoch in range(TOTAL_EPOCHS):
    ...
    for timestep in range(timesteps):
        ...
        terminal_flag = is_robot_on_ground() or is_state_truncated(timestep)
    ...
    if can_plot_rewards(epoch):
        ...
        plt.plot(x, [data["robot_height_reward"]
        ...
        plt.plot(x, [data["total_reward"]
                     for data in rewards_data_plot], color="black")

        plt.legend([
            "speed", "robot_fell_penalty", "robot_height_reward", "relative_distance_from_target", "foot_contact_readings",    "success_reward", "total_reward"])
        plt.savefig(
